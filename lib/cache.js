...
module.exports = class Cache {
  ...
  async cacheReleaseList(asset_id) {
    const { account, repository, token } = this.config
    const url = `https://api.github.com/repos/${account}/${repository}/releases/assets/${asset_id}`
    const headers = {
  ...
  async refreshCache() {
    ...
    for (const asset of release.assets) {
      const { name, browser_download_url, url, content_type, size, id } = asset
      ...
      if (name === 'RELEASES') {
        try {
          ...
          this.latest.files.RELEASES = await this.cacheReleaseList(id)
        } catch (err) {
          console.error(err)
        }
        continue
